...
- TextPad is an optional editor for `.txt` and `.md` files, separate from the memo library. Enable it in Settings → TextPad. `⌥⇧B` shows or hides the panel, reopening the current document during the configured reuse interval (15 minutes by default); ⌘N starts a new document after that interval. New documents save to Downloads or a chosen folder and copy their path. Automatic saving is on by default; turn it off for an unsaved scratch file that disappears when the panel loses focus. Open existing files from Finder or File → Open Text File…, use Save As to choose a name and location, Share to send the current text as a file, or Save to Memos to add a separate copy to the library.
