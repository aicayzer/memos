...
`Block` is `{ type: 'paragraph' }`, `{ type: 'heading', level }`, `{ type: 'codeBlock' }`, `{ type: 'quote' }`, `{ type: 'bulletList' }`, `{ type: 'orderedList' }` or `{ type: 'taskList' }`.
...
| Call                    | Effect                                                                             |
| ----------------------- | ---------------------------------------------------------------------------------- |
| `load(markdown)`        | Replace the document and put the caret at the start. Emits `state`, not `changed`. |
